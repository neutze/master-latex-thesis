...
%legitimate signature as well as correct \textit{classes.dex} structure cannot be verified are rejected for installation by the OS\newline
...
%optimization\newline
%step removes the \textit{classes.dex} from the original APK archive and loads in memory the .odex file upon execution, occurs only once, during the initial run of the application which explains the usually slower first application launch comparing to the subsequent ones
