\subsection{Code Abstraction Levels} \label{subsection:reengineering-abstraction}
...
It is used to represent the dex bytecode in a readable way and to identify the the result of the changes.
\newline
The third level is the Java code.
It gives the best overview of the code.
The result of the changes in the context of then method or class can be best identified in this presentation.
\newline
...
Analysing the dex bytecode allows to point out which characters in the bytecode have been modified.
\newline
...
The extracted \textit{classes.dex} is still contains the code in binary.
...
The syntax is loosely based on Jasmin's syntax
It takes the \gls{apk} and disassembles its \textit{classes.tex} file.
The output is a file smali file for each class.
...
The goal of the Java code is to reverse engineer the original code including the changes of the attacl.
It suits best for the analysis of the new behavior since it contains the most information, such as variable and method names.
...
\newline
DAD, short name of "DAD is A Decompiler".
...
JADX \cite{jadx} is the second compiler.
THe decompilation is directly done from the \gls{apk}'s dex bytecode to Java code.
It is the command line as seen in Code snippet~\ref{codeSnippet:jadxScript}.
\lstinputlisting[
  float=h,
  breaklines=true,
  captionpos=b,
